...
from signalbot.infrastructure.api_clients.sticker_packs_client import (
    StickerPacksClient,
)


@pytest.fixture
def sticker_packs_client(mock_session):
    return StickerPacksClient(mock_session, "http://localhost:8080")


@pytest.mark.asyncio
async def test_get_sticker_packs(
    sticker_packs_client: StickerPacksClient, mock_session
):
